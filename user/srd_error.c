...
int16  MAX_I		= 110;		// 50 means the MAX I is about 3A
...
void Error_Checking(void)
{
	...
	if(ERROR.CPU_Timer0_Remain < 0x0100)										ERROR.Code |= 0x0100;
	if(SRM_ANGLE<0||SRM_ANGLE>=3600)											ERROR.Code |= 0x0200;
	if(SRM_SPEED<0)																ERROR.Code |= 0x0400;
	if(SRM_SPEED>MAX_SPEED||SRM_SPEED<-MAX_SPEED)								ERROR.Code |= 0x0800;
	...
	if(ERROR.Code)
	{
		...
		if(ERROR.CPU_Timer0_Remain < 0X0100)	printf("PieCtrlRegs.PIEIFR1.all | 0x40) == 0 && CpuTimer0Regs.TIM.all < 0X0100\n");
